...
path.pure_pursuit_lookaheaddist = 20; % m - look ahead distance
...
% Elevation and slope along the track
path.zpath = zeros(npts,1);   % elevation (m) at each path point
path.s     = zeros(npts,1);   % distance along path (m)
path.dzds  = zeros(npts,1);   % slope dh/ds at each path point



i = 1; % counter for what data point we're at
% Starting point is on the first straight at x = 0, y = 0
% Height there is essentially 0 for the logistic curve
path.zpath(1) = 0;
    ...
    if path.xpath(i) < l_st % if the x component at the index of the counter is less than the straightaway length
        ...
        if path.xpath(i) >= 0 % if that x component is more than or equal to zero
            ...
            if path.ypath(i) < radius % if the y component at the counter index is less than the radius length
                ...
                x_new = path.xpath(i+1);
                y_new = path.ypath(i+1);

                % Logistic height going from 0 m to 10 m along x in [0, l_st]
                h = 10 * exp((x_new - 450)/50) / (1 + exp((x_new - 450)/50));
                ...
                % Elevation on second straight (return straight)
                x_new = path.xpath(i+1);
                y_new = path.ypath(i+1);
    
                % Mirror the logistic curve: height goes from 10 m back to 0 m
                % along the return straight. We mirror with (l_st - x_new).
                x_eff = l_st - x_new;
                h = 10 * exp((x_eff - 450)/50) / (1 + exp((x_eff - 450)/50));
            ...
            % Elevation on this curved section: connect straights smoothly
            x_new = path.xpath(i+1);
            y_new = path.ypath(i+1);

            if y_new < radius
                % lower curved section (between 0 m elevation straights)
                path.zpath(i+1) = 0;
            else
                % upper curved section (between 10 m elevation straights)
                path.zpath(i+1) = 10;
            end
        ...
        % Compute distance along path (s) for each point
path.s = (0:npts-1)' * delta_s;

% Compute numerical slope dz/ds along the path
if npts > 1
    path.dzds(1:end-1) = diff(path.zpath) / delta_s;
    path.dzds(end) = path.dzds(end-1);  % repeat last slope value
else
    path.dzds(:) = 0;
end
